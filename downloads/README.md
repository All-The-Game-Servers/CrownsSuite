...
Note: `CrownsMMO` and `CrownsTerrain` require the updated `CrownsAPI-1.3.0.jar` from this same folder, because the shared provider surfaces include MMO, terrain, suite status, and quest integration. The current resource pack is `1.5.0`, rebuilt for Minecraft `1.21.11`, so the MMO, terrain, quest, and suite status visuals resolve in-game. `CrownsMMO-1.7.0.jar` is the playable Floor 1 quest build. `CrownsTerrain-1.5.1.jar` is the reference-guided Floor 1 worldgen hotfix build and defaults to the fresh world `crowns_floor_1_v2`.
