...
Use `/cmmo start` to enter the configured dedicated Floor 1 world at First Haven. CrownsTerrain `1.5.4` uses the fresh world `crowns_floor_1_v4`; staff should run `/cterrain admin generate 1` and wait for `critical-ready` before sending players there.
