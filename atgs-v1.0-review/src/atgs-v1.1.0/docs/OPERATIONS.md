...
## Progenitor operator flow

The current Progenitor surface is intentionally operator-first:

- `Keepers`
  - live connectivity plus latest reported CPU, memory, and disk
- `Instances`
  - route state, lifecycle actions, and the official egg choices
- `Operations`
  - recent logs
  - bounded console input for running servers
  - recent task outcomes
  - recent audit history
- `Backups`
  - create, list, restore, delete
- `Schedules`
  - create and inspect backup schedules

The console action is intentionally narrow. It sends one line to a running server's stdin. It is not remote shell access.
...
- Progenitor `Operations` works against a real deployment, including logs and bounded console input
- backup restore paths are verified with `scripts/e2e_phase4.sh`
